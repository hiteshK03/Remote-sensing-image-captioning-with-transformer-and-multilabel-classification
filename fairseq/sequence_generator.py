...
class SequenceGenerator(object):
    ...
    def _generate(
        ...
        **kwargs
        ...
        encoder_outs_1 = model.forward_encoder(encoder_input)
        label_preds = model.forward_decoder_label(encoder_outs_1)

        new_order = torch.arange(bsz).view(-1, 1).repeat(1, beam_size).view(-1)
        new_order = new_order.to(src_tokens.device).long()
        encoder_outs = model.reorder_encoder_out(encoder_outs_1, new_order)
...
class EnsembleModel(torch.nn.Module):
    ...
    def __init__(self, models):
        ...
        if all(isinstance(m.decoder_cap, FairseqIncrementalDecoder) for m in models):
    ...
    @torch.no_grad()
    def forward_decoder_label(self, encoder_outs):
    ...
    def _decode_label(
        self, model, encoder_out
    ):
        if self.incremental_states is not None:
            decoder_label_out = list(model.forward_decoder_label(
                encoder_out=encoder_out
            ))
        else:
            decoder_label_out = list(model.forward_decoder_label(encoder_out=encoder_out))

        decoder_label_out = decoder_label_out[0]
        preds = torch.sigmoid(decoder_label_out)
        preds = np.array(preds.to(torch.float32).cpu())
        preds = np.array(preds > 0.4, dtype=np.float32)
        return preds
    ...
    def forward_decoder(self, tokens, encoder_outs, temperature=1.):
        ...
        for model, encoder_out in zip(self.models, encoder_outs):
            probs, attn, preds = self._decode_one(
                tokens,
                model,
                encoder_out,
                self.incremental_states,
    ...
    def _decode_one(
        ...
        if self.incremental_states is not None:
            decoder_cap_out = list(model.forward_decoder_cap(
                tokens, encoder_out=encoder_out, incremental_state=self.incremental_states[model],
            ))
        else:
            decoder_cap_out = list(model.forward_decoder_cap(tokens, encoder_out=encoder_out))

        decoder_cap_out[0] = decoder_cap_out[0][:, -1:, :]
        if temperature != 1.:
            decoder_cap_out[0].div_(temperature)
        attn = decoder_cap_out[1]
        if type(attn) is dict:
            attn = attn.get('attn', None)
        if attn is not None:
            attn = attn[:, -1, :]
        probs = model.get_normalized_probs(decoder_cap_out, log_probs=log_probs)
        probs = probs[:, -1, :]
    ...
    def reorder_incremental_state(self, new_order):
        ...
        for model in self.models:
            model.decoder_cap.reorder_incremental_state(self.incremental_states[model], new_order)
