...
from fairseq.models import FairseqDecoder, FairseqEncoder, FairseqDecoderLabel
...
class BaseFairseqModel(nn.Module):
    ...
    def get_targets(self, sample, net_output, decode):
    ...
    def get_normalized_probs_scriptable(
        ...
        if hasattr(self, "decoder_cap"):
            return self.decoder_cap.get_normalized_probs(net_output, log_probs, sample) 
        elif torch.is_tensor(net_output):
            logits_0 = net_output[0].float()
            logits_1 = net_output[1].float()
            if log_probs:
                return F.log_softmax(logits_0, dim=-1) + F.log_softmax(logits_1, dim=-1)
            else:
                return F.softmax(logits_0, dim=-1) + F.softmax(logits_1, dim=-1)
...
class FairseqEncoderDecoderModel(BaseFairseqModel):
    ...
    def __init__(self, encoder, decoder_cap, decoder_label):
        ...
        assert isinstance(self.decoder_cap, FairseqDecoder)
        assert isinstance(self.decoder_label, FairseqDecoderLabel)
    ...
    def forward(self, src_tokens, src_lengths, prev_output_tokens, **kwargs):
        ...
        decoder_cap_out_ = self.decoder_cap(
    ...
    def output_layer(self, features, **kwargs):
        ...
        return self.decoder_cap.output_layer(features, **kwargs)
