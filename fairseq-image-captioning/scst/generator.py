...
class SimpleSequenceGenerator:
    ...
    def generate(self, model, sample):
        ...
        dec_out, extra = model.decoder_cap(tokens, encoder_out=enc_out, incremental_state=incremental_state)
        ...
        incremental_state = model.decoder_cap.reorder_incremental_state(incremental_state, new_order)

        for _ in range(self.max_pos):
            tokens_batch = tokens.flatten(0, 1)  # (n x b, l)
            dec_out, extra = model.decoder_cap(tokens_batch, encoder_out=enc_out, incremental_state=incremental_state)
            ...
            lprobs_cat_masked = lprobs_cat * mask_cat[:, :, 1:-1].float()

            num_tokens = torch.sum(mask_cat[:, :, 1:-1], dim=2).float()
            ...
            incremental_state = model.decoder_cap.reorder_incremental_state(incremental_state=incremental_state,
        ...
        result_tokens = tokens[:, :, 1:].float() * result_mask.float()
        result_lprobs = lprobs * result_mask.float()

        result_num_tokens = torch.sum(result_mask, dim=2)
        result_scores = torch.sum(result_lprobs, dim=2) / result_num_tokens.float() ** self.penalty
